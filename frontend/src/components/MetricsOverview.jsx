...
export default function MetricsOverview({ activeSummary }) {
  if (!activeSummary) return null;

  const { total, highRisk } = activeSummary;
        ...
        <div className="glass-panel p-4 rounded-2xl relative overflow-hidden group hover:border-emerald-400 transition-all">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white mt-1">{total}</h3>
              <p className="text-[11px] text-emerald-600 dark:text-emerald-400 mt-1 flex items-center space-x-1 font-medium">
                <Users className="w-3 h-3" />
                <span>Pending or Snoozed, All Batches</span>
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-baseline space-x-2 mt-1">
                <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white">{highRisk}</h3>
